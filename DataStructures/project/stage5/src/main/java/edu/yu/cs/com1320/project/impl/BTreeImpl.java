...
//                    return entries[j];
                    ...
                    if(entries[j].getValue() == null){
//                        return null;
                        return entries[j];
                    }

                    if(entries[j].getValue().equals((Value) key)){
                        Entry deserializedVal = null;
                        try {
//                            deserializedVal = (Entry) this.pM.deserialize(key);
//                            return deserializedVal;
                            entries[j].val = this.pM.deserialize(key);
                            return entries[j];
                        } catch (IOException e) {
//                            e.printStackTrace();
                            return entries[j];
    ...
    public Value put(Key key, Value val) {
        ...
        if(alreadyThere != null) {
            Value oldVal = (Value) alreadyThere.val; //myAdd
...
//        return val; //need to figure out
    ...
    public void moveToDisk(Key k) throws Exception {
        this.pM.serialize(k, this.get(k));//check
        this.put(k, (Value) k);//try
    }

    @Override
    public void setPersistenceManager(PersistenceManager<Key, Value> pm) {
        this.pM = pm;//make sure this is okay
    }
}
